...
public static class WaveformRenderer
{
    ...
    // -----------------------------------------------------------------------------------------------
    // Canonical PCM WAV reader: 8-bit unsigned or 16-bit signed, mono or stereo -- the two shapes
    // WavWriter produces (voice lines are 8-bit). Anything else (24-bit, float, extensible) is rejected.
    // -----------------------------------------------------------------------------------------------
        ...
        try
        {
            ...
            while (stream.Position < stream.Length)
            {
                ...
                if (chunkId == "fmt ")
                {
                    ...
                    if (format != 1 || bitsPerSample is not (8 or 16) || channels is < 1 or > 2)
                        return ([], 0);
                }
            ...
            if (bitsPerSample == 8)
            {
                // Unsigned 8-bit, silence at 128: widen to signed 16-bit so the peak maths is shared.
                var widened = new short[sampleBytes.Length];
                for (int i = 0; i < sampleBytes.Length; i++)
                    widened[i] = (short)((sampleBytes[i] - 128) << 8);
                return (widened, channels);
            }
