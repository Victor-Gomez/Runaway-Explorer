...
public partial class SettingsPanel : UserControl
{
    ...
    private async void BrowseTNBT_Click(object? sender, RoutedEventArgs e)
    {
        if (_owner is null || _settings is null) return;
        string? folder = await Dialogs.ShowOpenFolderDialog(this, "Select The Next BIG Thing folder", _settings.TheNextBigThingDir);
        if (!string.IsNullOrWhiteSpace(folder))
        {
            _settings.TheNextBigThingDir = folder;
            _settings.RegisterRecentInstall(folder);
            _settings.Save();
            UpdateGamePaths();
            if (_settings.ActiveGame == GameVersion.TheNextBigThing)
                await _owner.ReloadActiveGameAsync();
        }
    }

    private async void ClearTNBT_Click(object? sender, RoutedEventArgs e)
    {
        if (_owner is null || _settings is null) return;
        _settings.TheNextBigThingDir = null;
        _settings.Save();
        UpdateGamePaths();
        if (_settings.ActiveGame == GameVersion.TheNextBigThing)
            await _owner.ReloadActiveGameAsync();
    ...
    private async void BrowseYesterday_Click(object? sender, RoutedEventArgs e)
    {
        if (_owner is null || _settings is null) return;
        string? folder = await Dialogs.ShowOpenFolderDialog(this, "Select Yesterday folder", _settings.YesterdayDir);
        if (!string.IsNullOrWhiteSpace(folder))
        {
            _settings.YesterdayDir = folder;
            _settings.RegisterRecentInstall(folder);
            _settings.Save();
            UpdateGamePaths();
    ...
    private async void ClearYesterday_Click(object? sender, RoutedEventArgs e)
    {
        if (_owner is null || _settings is null) return;
        _settings.YesterdayDir = null;
        _settings.Save();
        UpdateGamePaths();
        if (_settings.ActiveGame == GameVersion.Yesterday)
            await _owner.ReloadActiveGameAsync();
